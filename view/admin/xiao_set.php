...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $qqgroup = trim($_POST['qqgroup'] ?? '');
    $qqgrouplink = trim($_POST['qqgrouplink'] ?? '');
    ...
    if (empty($title) || empty($keywords) || empty($description) || empty($foot) || empty($logo) || empty($avatar) || empty($name) || empty($allowemail)) {
        ...
        $update_query = $conn->prepare(
            "UPDATE admins SET title = ?, keywords = ?, description = ?, announcement = ?, foot = ?, logo = ?, avatar = ?, name = ?, allowemail = ?, qqgroup = ?, qqgrouplink = ? WHERE id = 1"
        );
                ...
                <div class="card-body">
                    ...
                    <form action="" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="qqgroup" id="qqgroup" class="form-control" placeholder="请输入QQ群号" value="<?php echo htmlspecialchars($current_admin['qqgroup'] ?? '123456789'); ?>">
                            <small class="help-block">用于"加群获奖"功能，用户可通过加群获取完整获奖名单</small>
                        </div>
                        <div class="form-group">
                            <label for="qqgrouplink">QQ群链接</label>
                            <input type="url" name="qqgrouplink" id="qqgrouplink" class="form-control" placeholder="请输入QQ群加群链接" value="<?php echo htmlspecialchars($current_admin['qqgrouplink'] ?? 'https://qm.qq.com/cgi-bin/qm/qr?k=YOUR_GROUP_KEY'); ?>">
                            <small class="help-block">QQ群的一键加群链接，可在QQ群管理中获取</small>
                        </div>
                        <div class="form-group">
                            <label>网站logo</label>
