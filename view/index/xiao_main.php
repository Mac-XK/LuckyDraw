        ...
        <div class="card-body">
          <form action="" method="post" id="lotteryForm">
              <div class="form-group has-feedback feedback-left">
                  <input type="email" placeholder="请输入QQ邮箱（纯数字@qq.com）" class="form-control" name="email" id="email" required pattern="[0-9]+@qq\.com" title="请输入正确的QQ邮箱格式，如：123456789@qq.com" />
...
$(document).ready(function () {
    ...
    $('#send-code').on('click', function () {
        ...
        if (!validateQQEmail(email)) {
            $.alert({
                title: '邮箱格式错误',
                content: '请输入正确的QQ邮箱格式，如：123456789@qq.com（纯数字@qq.com）',
                type: 'red',
            });
            return;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!preg_match('/^[0-9]+@qq\.com$/', $account)) {
        showAlert('邮箱格式错误', '请输入正确的QQ邮箱格式，如：123456789@qq.com（纯数字@qq.com）');
        exit;
    }

    // 防刷检查
    $anti_spam_check = antiSpamCheck($conn, $_SESSION['admin_username'] ?? 'admin', $account, 'lottery');
    if (!$anti_spam_check['allowed']) {
        $error_msg = getAntiSpamMessage($anti_spam_check);
        showAlert($error_msg['title'], $error_msg['message']);
        exit;
    }
