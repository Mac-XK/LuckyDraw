...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_query = $conn->prepare("UPDATE admins SET username = ?, password = ? WHERE username = ?");
    $update_query->bind_param("sss", $new_username, $encrypted_password, $_SESSION['admin_username']);
    if ($update_query->execute()) {
        // 更新session中的用户名
        $_SESSION['admin_username'] = $new_username;
        echo "<script>alert('用户名和密码修改成功！'); window.location.href = '';</script>";
    } else {
                ...
                <div class="card-body">
                    <form action="" method="POST">
                        <div class="form-group has-feedback feedback-left">
                            <label>用户名</label>
                            <input type="text" name="username" class="form-control" value="<?php echo htmlspecialchars($current_admin['username']); ?>" required>
